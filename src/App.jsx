            ...
            <div className="space-y-6 animate-in fade-in duration-500 h-full flex flex-col">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-4 space-y-6">
                  <Card className="bg-capta-surface-low border-white/5 backdrop-blur-md">
                    <CardHeader className="border-b border-white/5 pb-4">
                      <CardTitle className="text-sm font-space uppercase tracking-[0.2em] flex items-center gap-2 text-capta-primary">
                        <MapPin size={16} /> Configuração do Robô
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="p-6 space-y-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-space text-slate-500 uppercase tracking-widest ml-1">O que buscar?</label>
                        <Input 
                          value={filtrosMaps.keyword}
                          onChange={(e) => setFiltrosMaps({...filtrosMaps, keyword: e.target.value})}
                          placeholder="Ex: Construtoras, Padarias..."
                          className="h-11 bg-capta-surface-lowest border-white/5 font-space"
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-space text-slate-500 uppercase tracking-widest ml-1">Onde buscar?</label>
                        <Input 
                      ...
                      <div className="pt-4 grid grid-cols-2 gap-3">
                        <Button 
                          onClick={() => handleStartMapsScan('robot')}
                          disabled={isScanningMaps}
                          className="bg-capta-primary text-capta-bg font-bold uppercase tracking-widest text-[10px] h-12 hover:shadow-[0_0_20px_rgba(47,217,244,0.3)]"
                        >
                          {isScanningMaps ? 'Rodando...' : 'Iniciar Robô'}
                        </Button>
                        <Button 
                          onClick={() => handleStartMapsScan('api')}
                          disabled={isScanningMaps}
                          variant="outline"
                          className="border-capta-primary/30 text-capta-primary font-bold uppercase tracking-widest text-[10px] h-12 hover:bg-capta-primary/10"
                        >
                          Usar API PRO
                        </Button>
                      </div>
                  ...
                  {/* Terminal de Logs */}
                  <Card className="bg-black/40 border-white/5 h-[350px] flex flex-col overflow-hidden">
                    ...
                    {isScanningMaps && (
                       <div className="p-3 border-t border-white/5 bg-white/[0.02]">
                         <div className="h-1 bg-white/5 w-full rounded-full overflow-hidden">
                            <div className="h-full bg-capta-primary animate-[progress_2s_ease-in-out_infinite]" style={{width: '30%'}}></div>
                         </div>
                       </div>
                    )}
                  </Card>
                ...
                <div className="lg:col-span-8 flex flex-col gap-6">
                  <div className="flex items-center justify-between bg-capta-surface-low border border-white/5 p-4">
                    <div className="flex items-center gap-4">
                       <div className="px-3 py-1 bg-capta-primary/10 border border-capta-primary/20 text-capta-primary text-[10px] font-space uppercase tracking-widest">
                         Base Extraída: {storedGmnLeads.length}
                       </div>
                       {isScanningMaps && (
                         <div className="text-[10px] font-space text-slate-500 uppercase tracking-widest animate-pulse">
                           Processados: {mapsProgress.processed} / {mapsProgress.total}
                         </div>
                       )}
                    </div>
                    <Button 
                      ...
                      variant="ghost" 
                      className="text-red-400 hover:bg-red-400/10 text-[9px] font-space uppercase tracking-widest h-8"
                    >
                      Limpar Base
                    </Button>
                  </div>

                  <div className="bg-capta-surface-low/30 border border-white/5 flex-1 min-h-[500px] overflow-hidden flex flex-col">
                    {storedGmnLeads.length === 0 ? (
                      <div className="flex-1 flex flex-col items-center justify-center text-slate-600 gap-4">
                         <MapPin size={48} className="opacity-20" />
                         <span className="font-space text-[10px] uppercase tracking-[0.3em] opacity-40">Nenhum Lead na Fila de Processamento</span>
                      </div>
                    ) : (
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                          <thead>
                            <tr className="border-b border-white/5 bg-white/[0.02] text-[9px] font-space text-slate-500 uppercase tracking-widest">
                              <th className="p-4">Empresa</th>
                              <th className="p-4">Contato Extraído</th>
                              <th className="p-4">Site / Social</th>
                          ...
                          <tbody className="divide-y divide-white/5 font-mono text-[11px] text-slate-300">
                            {storedGmnLeads.map((lead, i) => (
                              <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                                <td className="p-4">
                                  <div className="font-space font-bold text-white uppercase truncate max-w-[200px]">{lead["Nome Empresa"]}</div>
                                  <div className="text-[9px] text-slate-500 truncate max-w-[200px]">{lead["Endereço"]}</div>
                                </td>
                                <td className="p-4">
                                  <div className="flex flex-col gap-1">
                                    <span className="text-capta-primary">{lead["Telefone 1"] || 'N/A'}</span>
                                    <span className="text-[9px] text-slate-500">{lead["E-mail"] || 'Sem e-mail'}</span>
                                  </div>
                                </td>
                                <td className="p-4">
                                ...
                                <td className="p-4">
                                  <span className="px-2 py-0.5 bg-green-500/10 text-green-400 border border-green-500/20 text-[9px] font-space uppercase tracking-widest">Extraído</span>
                                </td>
                                <td className="p-4 text-right">
                                   <Button 
                                     onClick={() => moveToCRM(lead, 'Maps')}
                                     className="h-8 px-4 bg-capta-primary/10 hover:bg-capta-primary text-capta-primary hover:text-capta-bg font-space text-[9px] uppercase tracking-widest border border-capta-primary/30"
                                   >
                                     CRM
                                   </Button>
                                </td>
